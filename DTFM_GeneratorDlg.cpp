...
int NeedUpdateMidiEvent=1;	//обновл€ть Midi сообщени€
...
bool Overload=false; //перегрузка звуков. уровн€ >32767 OR <-32767
...
struct KEY
{
	...
	double t;	//врем€(в условных единицах) сигнала
	double A,D,S,R;	//блок ADSR (Attack-Decay-Sustain-Release)
	double A_add;	//скорость увеличени€ A
...
//функци€ используетс€ дл€ генерации звука
...
//freq_actual записываетс€ последн€€ вычисленна€ частота
...
double Piano(double Ampl, double freq, double t, double phase, int & flag_one, double & freq_actual)
{
	...
	double AMP=Ampl;
	AMP+=
		5000//глубина модул€ции
		*sin(modulation);

	double f;	//частота

	//если подн€т первый слайдер (базова€ гармоника)
...
BOOL CDTFM_GeneratorDlg::PreTranslateMessage(MSG* pMsg) 
{
	...
	if (pMsg->message == MM_WOM_DONE)
	{
		...
		//добавл€ем в очередь новый буфер!!!!!!!!!!!!
...
//	GetData;
	...
	//если генерируетс€ чисто синусоидальный сигнао
...
void CDTFM_GeneratorDlg::OnButtonMidiClose() 
{
	if (hmidiIn!=NULL)
	{
		...
		if (err!=MMSYSERR_NOERROR) 
		{
			m_midi_name="ќшибка midiInClose";
		}
	...
	if (write)
	{
			...
			if (value>0)
			{
				if (file.Open("notes.h", file.modeWrite))
				{
					...
					CString s;
					//записываем врем€ от отпускани€ предыдущей до нажати€ этой
					s.Format("{0, %i},\n", md.time-last_time);
			...
			if (value==0)
			{
				...
				if (file.Open("notes.h", file.modeWrite))
				{
					...
					CString s;
					//записываем длительность нажати€ клавиши
